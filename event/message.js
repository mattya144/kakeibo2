...
const dbAPI = 'https://sheetdb.io/api/v1/9to86hvucal4t';
// テキストメッセージの処理をする関数
const textEvent = async (event, client) => {
    ...
    if (data) {
        ...
        if ((data.context === 'addMode')&&(data.context_2 === 'dailyMode')) {
            ...
            await axios.put(`${dbAPI}/userId/${userId}`, { data: [{ context_2: '' }] });
            await axios.post(dbAPI, { data: [{ 日付: Addinfo_date, 日用品: Addinfo[0], 値段: Addinfo_price }] });
            // index関数に返信するメッセージを返す
            return {
                type: 'text',
                text: `${event.message.text}　を追加しましたわ`
            };
        }else if ((data.context === 'addMode')&&(data.context_2 === 'foodMode')) {
            // DBへメッセージのデータを追加してcontextを空にする
            const Addinfo = event.message.text.split("　");
            const Addinfo_price = Addinfo[1].replace("円", "");
            await axios.put(`${dbAPI}/userId/${userId}`, { data: [{ context: '' }] });
            await axios.put(`${dbAPI}/userId/${userId}`, { data: [{ context_2: '' }] });
            await axios.post(`${dbAPI}`, { data: [{ 日付: Addinfo_date, 食料品: Addinfo[0], 値段: Addinfo_price }] });
            // index関数に返信するメッセージを返す
            return {
                type: 'text',
                text: `${event.message.text}を追加しましたわ`
    ...
    switch (event.message.text) {
        ...
        case '残高': {
            ...
            if (data) {
                // 返信するメッセージを作成
                //const money_use = await axios.get(`${dbAPI}?sheet=sheet2/cells/E1`)
                message = {
                    type: 'text',
                    text: `今月は残り${data.金額設定}円使えるよ`,
                };
            } else {
                // 返信するメッセージを作成
                message = {
        ...
        case '日用品' : {
            if (data) {
                ...
                await axios.put(`${dbAPI}/userId/${userId}`, { data: [{ context_2: 'dailyMode' }] });
            } else {
                await axios.post(dbAPI, { data: [{ userId, context: 'addMode' }] });
                await axios.post(dbAPI, { data: [{ userId, context_2: 'dailyMode' }] });
            }
            message = {
                type: 'text',
                text: 'わかった！日用品だね\n商品名と値段を空白切りにして入力してね'
        ...
        case '食料品' : {
            if (data) {
                await axios.put(`${dbAPI}/userId/${userId}`, { data: [{ context: 'addMode' }] });
                await axios.put(`${dbAPI}/userId/${userId}`, { data: [{ context_2: 'foodMode' }] });
            } else {
                await axios.post(dbAPI, { data: [{ userId, context: 'addMode' }] });
                await axios.post(dbAPI, { data: [{ userId, context_2: 'foodMode' }] });
            }
            message = {
                type: 'text',
                text: 'わかった！食料品だね\n商品名と値段を空白切りにして入力してね'
